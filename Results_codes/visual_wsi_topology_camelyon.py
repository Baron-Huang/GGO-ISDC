...
if __name__ == '__main__':
    ...
    ### single mode
    feats_full = joblib.load(r'/root/autodl-tmp/GGO_ISDC_public/Results/Features/CAMELYON16/GGO_ISDC/feats_full.joblib')
    relations = joblib.load(r'/root/autodl-tmp/GGO_ISDC_public/Results/Relations/CAMELYON16/GGO_ISDC/relations.joblib')
    labels = joblib.load(r'/root/autodl-tmp/GGO_ISDC_public/Results/Relations/CAMELYON16/GGO_ISDC/labels.joblib')

    inst_i = 0
    min_clus_show_num = 25
    max_clus_show_num = 10
    adje_value = 0.95
    offsets = [
        (0.39, 0.4),  # Cluster 0 标签放右上
        (-0.19, 0),  # Cluster 1 标签放左上
    ]
    ...
    save_root_path = r'/root/autodl-tmp/GGO_ISDC_public/Results/Visual/CAMELYON16/GGO_ISDC/Topology_map'
    if not os.path.exists(save_root_path):
        os.makedirs(save_root_path)

    save_path = os.path.join(save_root_path, 'wsi_topology_camelyon_' + str(inst_i) + '.jpg')
