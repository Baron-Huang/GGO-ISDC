...
def other_paras_cfg():
    ...
    paras.add_argument('--pretrained_weights_path', type=str,
                    default='/root/autodl-tmp/GGO_ISDC_public/Pretrained_wegts/Swin_trans/swin_small_patch4_window7_224_22k.pth')
...
def base_paras_cfg():
    ...
    paras.add_argument('--num_classes', type=int, default=2)
...
def extract_features_for_swint(base_args=None, other_args=None, save_path=None, read_path = None, gpu_device = 1):
    ...
    with torch.no_grad():
        ...
        for img_data_list, img_label in train_loader:
            ...
            for img_data in img_data_list:
                if img_data.shape[0] <= input_len:
                    ...
                    for img_data_i in range(xxx_count):
                        kkk = feats_extract_model(img_data[input_len*img_data_i:(input_len*(img_data_i+1)), :, :, :].cuda(gpu_device))
            ...
            np.save(os.path.join(feats_path, str(count_save) + '.npy'), swint_feats_mat)
            np.save(os.path.join(labels_path, str(count_save) + '.npy'), label)
            print(count_save)
